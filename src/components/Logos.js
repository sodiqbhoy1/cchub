import Airbus from  '../assets/airbus.png';
import AWS from '../assets/aws-logo.png';
import BBC from '../assets/bbc.png';
import Billgates from '../assets/billgates.png'
import Facebook from '../assets/facebook.png';
import Ford from '../assets/ford-foundation.png';
import Google from '../assets/google.png';
import Indigo from '../assets/indigo.png';
import Microsoft from '../assets/microsoft.png';
import MacArthur from '../assets/mac-arthur-logo.png';
import Mainone from '../assets/mainone.png';
import Omidyar from '../assets/omidyar.png';
import Raspberry from '../assets/raspberry.png';
    
        const Logos = [
            {id:1, src: Airbus, alt: "Airbus" },
            {id:2, src: AWS, alt: "AWS" },
            {id:3, src: BBC, alt: "BBC" },
            {id:4, src: Billgates, alt: "Bill Gates Foundation" },
            {id:5, src: Facebook, alt: "Facebook" },
            {id:6, src: Ford, alt: "Ford Foundation" },
            {id:7, src: Google, alt: "Google" },
            {id:8, src: Indigo, alt: "Indigo" },
            {id:9, src: Microsoft, alt: "Microsoft" },
            {id:10, src: MacArthur, alt: "MacArthur Foundation" },
            {id:11, src: Mainone, alt: "MainOne" },
            {id:12, src: Omidyar, alt: "Omidyar Network" },
            {id:13, src: Raspberry, alt: "Raspberry Pi Foundation" }
        ];
export default Logos;